...
function tsDraw(p_id, p_table)
{
	...
	var row = rows[0];
	var cols = row.getElementsByTagName('td');
	    temp =parseInt(cols[0].innerHTML.trim());
	   //var temp =row[0].valueOf().cells[0].valueOf().innerHTML;
	}catch(e){
			alert('tsDraw'+e);
			}
	...
	if	(classes == null)
	{
		for (i = 0; i < len; i++){
			try{
			row_clones[i].valueOf().cells[0].valueOf().innerHTML =temp;
			temp =temp+1;
	...
	else
	{
		...
		{   try{
			...
			clone[i].valueOf().cells[0].valueOf().innerHTML =temp;
			temp =temp+1;
...
function ingnoreHTML(str)
			{
				var stripTagsRE    = /<\/?[^>]+>/gi,
				s = String(str);
				return s.replace(stripTagsRE,"");
			}
