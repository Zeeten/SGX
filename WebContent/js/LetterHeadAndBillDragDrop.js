...
try
{
document.getElementById((dd.obj.name)+"Left").value=dd.obj.x;
document.getElementById((dd.obj.name)+"Top").value=dd.obj.y;


document.getElementById((dd.obj.name)+"Height").value=dd.obj.h;
document.getElementById((dd.obj.name)+"Width").value=dd.obj.w;
...
if(dd.obj.is_image)
{
   ...
   window.setTimeout(onLoadLetterHeadxx,100);
   function onLoadLetterHeadxx()
   {
 
   for(i=0;i<dd.elements.length;i++)
   {
   if(dd.elements[i].is_image)
{

xx=dd.elements[i].name.substring("divNames".length);
document.getElementById(dd.elements[i].id).style.opacity=document.getElementById('forChangeOpacity'+xx).value;
