...
<%@page import="in.co.sunrays.util.AccessUtility"%>
   ...
   <script src="<%=ORSView.JS_FOLDER%>/assessordropdown.js"></script> 
   <%--   <script src="<%=ORSView.JS_FOLDER%>/assessorvalidation.js"></script>  --%>
    <script>
    $(function () {
...
<br>
			<p>
<font class="textPMWithOutBorderCommonCenterRedBoldUnderline">Assessor</font>
</p>
	<table border="1" cellspacing="0" style="border-collapse: collapse;border: 1px solid gray;" width="100%">
	<tr>
		<td>
			<form  name = "form" action="<%=ORSView.ASSESSOR_CTL%>" method="POST">
...
<table style="margin-right:-760px;margin-top:-870px;">
			...
			if (imageName != null && imageName.trim().length() > 0) {
				imageName = ORSView.APP_CONTEXT + "/" + AppRole.ASSESSOR + "/"
						+ imageName;
			} else {
...
<tr>	<td>
	...
	<img src="<%=imageName%>" height="100px" width="100px" style="margin-top:-20px;"> <br>
			<form method="POST" enctype="multipart/form-data"
				action="<%=ORSView.FILE_UPLOAD_CTL%>">
				<input type="hidden" name="id" value="<%=model.getId()%>"> <input
					type="hidden" name="type" value="<%=AppRole.ASSESSOR%>">
				<input type="hidden" name="page"
					value="<%=ORSView.ASSESSOR_CTL%>?id=<%=model.getId()%>"><br />
					<% if(model.getId()>0) { %>
