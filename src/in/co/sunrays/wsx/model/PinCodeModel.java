...
public class PinCodeModel extends BaseModel {

	private static Logger log = Logger.getLogger(PinCodeModel.class);
	...
	public List search(int pageNo, int pageSize) throws ApplicationException {
		...
		StringBuffer sql = new StringBuffer(
				"SELECT * FROM PIN_CODE WHERE 1=1");
		...
		if (postOfficeId > 0) {
			...
			sql.append(" AND POST_OFFICE_ID = " + postOfficeId);
		}
		...
		try {
			...
			while (rs.next()) {
				...
				model.setPostOfficeId(rs.getLong(3));	
				model.setDistrictId(rs.getLong(4));	
				model.setStateId(rs.getLong(5));	
				model.setCountryId(rs.getLong(6));		
