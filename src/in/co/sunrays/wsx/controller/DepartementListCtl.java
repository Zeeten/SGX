...
public class DepartementListCtl extends BaseCtl {
	...
	protected void doGet(HttpServletRequest request,
		...
		log.debug("DepartementListCtl doGet Start");
		...
		try {
			...
			if (OP_SEARCH.equalsIgnoreCase(op) || "Next".equalsIgnoreCase(op)
				...
				ServletUtility.redirect(ORSView.DEPARTEMENT_CTL, request,
						response);
				return;
			} else if (OP_DELETE.equalsIgnoreCase(op)) {
				pageNo = 1;
				if (ids != null && ids.length > 0) {
					DepartementModel deletemodel = new DepartementModel();
					for (String id : ids) {
		...
		log.debug("DepartementListCtl doGet End");
	}

	@Override
	protected String getView() {
		return ORSView.DEPARTEMENT_LIST_VIEW;
	}
