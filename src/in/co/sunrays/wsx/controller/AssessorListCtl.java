...
public class AssessorListCtl extends BaseCtl {
	...
	private static Logger log = Logger.getLogger(AssessorListCtl.class);
	...
	protected void doGet(HttpServletRequest request,
		...
		log.debug("AssessorListCtl doGet Start");
		...
		try {
			...
			if (OP_SEARCH.equalsIgnoreCase(op) || "Next".equalsIgnoreCase(op)
				...
				ServletUtility.redirect(ORSView.ASSESSOR_CTL, request, response);
				return;
			} else if (OP_DELETE.equalsIgnoreCase(op)) {
				pageNo = 1;
				if (ids != null && ids.length > 0) {
					AssessorModel deletemodel = new AssessorModel();
					for (String id : ids) {
		...
		log.debug("AssessorListCtl doGet End");
	}

	@Override
	protected String getView() {
		return ORSView.ASSESSOR_LIST_VIEW;
	}
