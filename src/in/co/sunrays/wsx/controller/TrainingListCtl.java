...
public class TrainingListCtl extends BaseCtl {
	...
	protected void doGet(HttpServletRequest request,
		...
		log.debug("TrainingManagementListCtl doGet Start");
		...
		try {
			...
			if (OP_SEARCH.equalsIgnoreCase(op) || "Next".equalsIgnoreCase(op)
				...
				ServletUtility.redirect(ORSView.TRAININGMANAGEMENT_CTL,
						request, response);
				return;
			} else if (OP_DELETE.equalsIgnoreCase(op)) {
				pageNo = 1;
				if (ids != null && ids.length > 0) {
					TrainingModel deletemodel = new TrainingModel();
					for (String id : ids) {
		...
		log.debug("TrainingManagementListCtl doGet End");
	}

	@Override
	protected String getView() {
		return ORSView.TRAININGMANAGEMENT_LIST_VIEW;
	}
